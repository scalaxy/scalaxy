                                          ...
                                          (let ((entry (gethash key (s3-config-lazy-index cfg))))
                                            (when (and (node-owned-p node key)
                                              ...
                                              (let* ((raw (car (multiple-value-list (%codec-read bytes p2))))
                                                     (rec (ignore-errors (%decode-record raw)))
                                                     (rtype (and rec (%record-get rec "type")))
                                                     (start (and rec (princ-to-string (%record-get rec "start"))))
                                                     (end (and rec (princ-to-string (%record-get rec "end")))))
