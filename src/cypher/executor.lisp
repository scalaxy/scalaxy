                          ...
                          (ecase (car el)
                            ...
                            (:rel
                             (let* ((next-el (nth (1+ idx) elements))
                                    ...
                                    (bound-list (and rvar (assoc rvar r)
                                                     (cypher-list-p (cdr (assoc rvar r)))
                                                     (cdr (assoc rvar r)))))
                               (let* ((min-hops (if bound-list
                                                    (length (cypher-list-elements bound-list))
                                                    min-hops))
                                      (max-hops (if bound-list
                                                    (length (cypher-list-elements bound-list))
                                                    max-hops))
                                      (queue (list (list r 0 path visited nil))))
                                 (loop while queue
  ...
  (stable-sort
   ...
   (lambda (a b)
     (loop for va in (car a) for vb in (car b)
           ...
           for c = (%sort-compare va vb)
           do (cond ((eq c :null) (return (and (not (cypher-null-p va))
                                               (cypher-null-p vb))))
                    ((eq c :eq))
...
(defun %fast-count-query (ast graph)
  "Fast path only for semantically exact, unfiltered whole-graph counts."
    ...
    (let* ((clauses (rest ast)) (match (first clauses)) (ret (second clauses)))
      (when (and match ret (eq (car match) :match) (eq (car ret) :return)
        ...
        (let* ((item (first (getf (cdr ret) :items)))
               ...
               (node-p (lambda (x)
                         (and (consp x) (eq (car x) :node)
                              (null (getf (cdr x) :labels))
                              (null (getf (cdr x) :props)))))
               (rel-p (lambda (x)
                        (and (consp x) (eq (car x) :rel)
                             (null (getf (cdr x) :type))
                             (null (getf (cdr x) :types))
                             (null (getf (cdr x) :props)))))
               (value
                 (cond
                   ((and (consp expr) (string-equal (getf (cdr expr) :fn) "count")
                         (= (length (getf (cdr expr) :args)) 1)
                         (or (eq arg :*) (symbolp arg))
                         (consp chain) (= (length chain) 1)
                         (funcall node-p (first chain))
                         (or (eq arg :*) (equal arg (getf (cdr (first chain)) :var))))
                    (if (typep graph 'gateway-graph-view)
                        (gateway-graph-count (graph-gateway graph) :nodes :db (graph-db graph))
                        (graph-count-nodes graph)))
                   ((and (consp expr) (string-equal (getf (cdr expr) :fn) "count")
                         (= (length (getf (cdr expr) :args)) 1)
                         (or (eq arg :*) (symbolp arg))
                         (consp chain) (= (length chain) 3)
                         (funcall node-p (first chain))
                         (funcall rel-p (second chain))
                         (funcall node-p (third chain))
                         ;; Do not collapse self-variable or typed patterns.
                         (not (and (getf (cdr (first chain)) :var)
                                   (equal (getf (cdr (first chain)) :var)
                                          (getf (cdr (third chain)) :var))))
                         (or (eq arg :*)
                             (equal arg (getf (cdr (second chain)) :var))))
                    (if (typep graph 'gateway-graph-view)
                        (gateway-graph-count (graph-gateway graph) :rels :db (graph-db graph))
                        (graph-count-rels graph)))
                   (t nil))))
          (when value (list (list (cons (%item-name item) value)))))))))
                 ...
                 (lambda (row exists-form)
                   (if (eq (car exists-form) :exists-sub)
                       (let ((sub (getf (cdr exists-form) :clauses))
                             (chain (getf (cdr exists-form) :chain))
                             (where (getf (cdr exists-form) :where)))
                         (if chain
                             (if where
                                 (let ((matches (cursor-drain
                                                 (%pattern-cursor
                                                  graph (%list-cursor (list row))
                                                  (list chain) graph params))))
                                   (if (some (lambda (r)
                                               (%tv-true (eval-expr where r graph params)))
                                             matches)
                                       t nil))
                                 (%pattern-exists row (list chain) graph params))
                             (if (and sub
