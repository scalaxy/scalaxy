...
(defun %duration-between (lhs rhs &optional mode)
  "Duration from LHS to RHS (same kinds).  MODE: nil (between), or
:months / :days / :seconds."
  (unless (and (%temporal-p lhs) (%temporal-p rhs) (eq (car lhs) (car rhs)))
    (cypher-signal "InvalidArgumentType" :detail "duration.between needs two same-kind temporals"))
  (let ((dn (- (%to-epoch-ns rhs) (%to-epoch-ns lhs))))
    (cond
      ((or (eq mode :seconds) (null mode))
       (multiple-value-bind (ds rns) (floor (/ dn (* 86400 1000000000)))
         (multiple-value-bind (sec rem) (floor rns 1000000000)
           (list :cypher-duration :months 0 :days ds :nanos (+ (* sec 1000000000) rem)))))
      ((eq mode :days)
       (let ((ndays (round (/ dn (* 86400 1000000000)))))
         (list :cypher-duration :months 0 :days ndays :nanos 0)))
      ((eq mode :months)
       (let ((l (cdr lhs)) (r (cdr rhs)))
         (let ((ym (- (getf r :year) (getf l :year)))
               (mm (- (or (getf r :month) 1) (or (getf l :month) 1))))
           (let ((tot (+ (* ym 12) mm)))
             (list :cypher-duration :months tot :days 0 :nanos 0))))))))
