...
Boolean results use the T/:CYPHER-FALSE convention, never CL NIL."
  (flet ((b (x) (if x t :cypher-false)))
    (cond
      ...
      ((and (%path-p a) (%path-p b))
       ;; path equality compares the node/relationship ids (traversal
       ;; direction is not significant)
       (b (equal (mapcar (lambda (x) (if (and (consp x) (getf x :id))
                                         (getf x :id) x))
                         (second a))
                 (mapcar (lambda (x) (if (and (consp x) (getf x :id))
                                         (getf x :id) x))
                         (second b)))))
    ...
    ((and (cypher-list-p a) (cypher-list-p b))
     (let ((as (cypher-list-elements a)) (bs (cypher-list-elements b)))
       (loop for x in as for y in bs
             ...
             do (cond ((member c '(:lt :gt)) (return c))
                      ((eq c :null) (return :null)))
             finally (return (cond ((< (length as) (length bs)) :lt)
                                   ((> (length as) (length bs)) :gt)
                                   (t :eq))))))
    (t :null)))
  ...
  (flet ((a0 () (first args))
    ...
    (let ((arg0 (a0)) (arg1 (a1)))
      (cond
        ...
        ((string-equal name "toString")
         (if (%tv-null arg0) :cypher-null (%to-string arg0)))
        ...
        ((string-equal name "startNode")
         (cond ((%rel-p arg0)
                (or (getf arg0 :start-node)
                    (graph-node graph (getf arg0 :start))))
               ((%tv-null arg0) :cypher-null)
               (t (%fn-error name args))))
        ((string-equal name "endNode")
         (cond ((%rel-p arg0)
                (or (getf arg0 :end-node)
                    (graph-node graph (getf arg0 :end))))
               ((%tv-null arg0) :cypher-null)
               (t (%fn-error name args))))
  ...
  (case op
    ...
    ((:< :> :<= :>=)
     (if (and (numberp a) (numberp b)
              (or (and (numberp a) (/= a a))
                  (and (numberp b) (/= b b))))
         ;; NaN compares false against every number (openCypher)
         :cypher-false
