  ...
  (cond
    ...
    ((%tck-rel-p text)
     (handler-case (%tck-parse-entity text)
       (error () (format nil "<unparsable: ~a>" text))))
    ((and (>= (length text) 2)
          (char= (char text 0) (code-char 123))
          (char= (char text (1- (length text))) (code-char 125)))
     ;; map literal containing entity literals ({node1: (:A), rel: [:T]})
     (let ((pairs nil))
       (dolist (seg (%tck-split-top-level (subseq text 1 (1- (length text)))))
         (unless (string= (string-trim " " seg) "")
           (let ((k (position (code-char 58) seg)))
             (if k
                 (push (cons (string-trim " " (subseq seg 0 k))
                             (%tck-parse-cell (string-trim " " (subseq seg (1+ k)))))
                       pairs)
                 (push (cons (string-trim " " seg) nil) pairs)))))
       (cypher-map (nreverse pairs))))
    (t
...
Returns T or NIL."
  (cond
    ...
    ((and (scalaxy::%temporal-p actual) (stringp expected))
     (string= (scalaxy::%temporal-to-string actual) expected))
    ((and (scalaxy::%temporal-p actual) (scalaxy::%temporal-p expected))
     (string= (scalaxy::%temporal-to-string actual)
              (scalaxy::%temporal-to-string expected)))
    ((and (consp expected) (eq (car expected) :node))
...
engine does not implement, or NIL."
  (let ((up (string-upcase q)))
    (cond
      ...
      ;; temporal functions are implemented (src/cypher/temporal.lisp)
