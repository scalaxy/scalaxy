...
(defun test-cypher-lexer ()
  (deftest cypher-lexer
    ...
    (check (equal (cytoken-value (car (cypher-lex "4.5"))) 4.5d0) "lexer float")
    (check (equal (cytoken-value (car (cypher-lex "1e3"))) 1000.0d0) "lexer exponent")
...
(defun test-temporal ()
  (deftest temporal
    (let* ((store (make-store))
           (g (make-local-graph store)))
      (labels ((one (q) (cdr (assoc (ast-var "r") (first (cypher-query q g))))))
        (let ((d (one "RETURN date('2015-W30-2') AS r")))
          (check (scalaxy::%date-p d) "temporal: date from week string")
          (check (equal (list (getf (cdr d) :year) (getf (cdr d) :month) (getf (cdr d) :day))
                        '(2015 7 21))
                 "temporal: week date resolves"))
        (check (equal (one "RETURN toString(date({year:1984, month:10, day:11})) AS r") "1984-10-11")
               "temporal: toString of map date")
        (check (equal (one "RETURN date('2015-07-21').dayOfWeek AS r") 2)
               "temporal: dayOfWeek accessor")
        (check (eq (one "RETURN date('2015-07-21') < date('2016-01-01') AS r") t)
               "temporal: date comparison")
        (let ((dd (one "RETURN localdatetime('1984-03-07T12:31:14.645') AS r")))
          (check (scalaxy::%localdatetime-p dd) "temporal: localdatetime from string"))
        ;; codec round-trip through graph props
        (graph-create-node g :labels '("N") :props '(("d" . (:cypher-date :year 2015 :month 7 :day 21))))
        (let ((node (scalaxy::graph-node g "g0000000000000001")))
          (check (equal (cdr (assoc "d" (getf node :props) :test #'equal))
                        (list :cypher-date :year 2015 :month 7 :day 21))
                 "temporal: codec round-trip"))))))
